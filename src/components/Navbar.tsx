...
interface Props {
  ...
  setvside: Dispatch<SetStateAction<string>>;
  side: string;
  vside: string;
  play: string;
...
const Hamburger = ({ state, setstate, on, off, side }: { state: string, setstate: Dispatch<SetStateAction<string>>, on: string, off: string, side:boolean }) => {
  ...
  return (
    <nav className="fixed top-0 left-0 w-full bg-white shadow-md z-50">
      <div className="container flex justify-between items-center p-4">
        ...
        <div className="flex items-center justify-between space-x-2 w-1/6 ml-0">
          <Hamburger state={side} setstate={setside} on="left-0" off="-left-[25%]" side={true} />
          <div className="text-2xl  text-red-500 font-bold">YouTube</div>
        </div>

        <div className="flex justify-between w-[35%] absolute right-10 items-center">
          ...
          <span className={`${play?"":"invisible"}`} >
            <Hamburger state={vside} setstate={setvside} on="right-0" off="-right-[21%]" side={false} />
            &nbsp;&nbsp;
